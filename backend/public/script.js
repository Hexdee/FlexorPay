...
document.getElementById("token").addEventListener("change", async (e) => {
    ...
    try {
        const allowance = Number(await tokenContract.methods.allowance(account, flexorpayAddress).call());
...
const approveToken = async () => {
    ...
    try {
        ...
        const tokenContract = await contract(erc20Abi, tokenAddress);
        const tx = await tokenContract.methods.approve(flexorpayAddress, "0xffffffffffffffffffffffffffffffff").send({ from: account });
        await tx.wait;
        payBtn.style.display = "block";
        approveBtn.style.display = "none";
    }
...
const requiredTokenAmount = async (token, amountInUSD) => {
    const flexorpay = await contract(flexorpayAbi, flexorpayAddress);
    const amount = await flexorpay.methods.requiredTokenAmount(amountInUSD * stableCoinDecimals, token).call();
    return amount;
}
...
const makePayment = async () => {
    ...
    if (token && name && amount && desc) {
        try {
            ...
            const flexorpay = await contract(flexorpayAbi, flexorpayAddress);
            if (token != address0) {
                const tx = await flexorpay.methods.makePayment(token, amount * stableCoinDecimals, name, desc, merchant).send({ from: account });
                await tx.wait;
            } else {
                const requireAmount = await requiredTokenAmount(weth, amount);
                const tx = await flexorpay.methods.makePayment(token, amount * stableCoinDecimals, name, desc, merchant).send({ from: account, value: requireAmount });
            }
            ...
            console.log(err);
            alert("An error occured");
            payBtn.innerText = "Pay with FlexorPay";
        }
    } else if (!amount) {
